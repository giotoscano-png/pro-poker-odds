...
export default function OddsDisplay({ odds, isCalculating }) {
  ...
  return (
    <div className="odds-stack">
      ...
      <OddsBar label="Vittoria" value={odds?.win || 0} tone="win" icon={<TrendingUp size={16} />} isCalculating={isCalculating} />
      <OddsBar label="Pareggio" value={odds?.tie || 0} tone="tie" icon={<Minus size={16} />} isCalculating={isCalculating} />
      <OddsBar label="Sconfitta" value={odds?.lose || 0} tone="loss" icon={<TrendingDown size={16} />} isCalculating={isCalculating} />
    </div>
  );
}
