  ...
  if (!analysis) {
    return (
      <div className="empty-state">
        <Info size={18} />
        Disponibile dopo il Flop, cioè con 3 carte sul tavolo.
      </div>
    );
  }
  ...
  return (
    <div className="outs-stack">
      {drawDetails.length > 0 && (
        <div>
          <p className="tiny-title">Progetti in corso</p>
      ...
      <div className="outs-grid">
        <div className="outs-total">
          <div>
            <Target size={15} />
            <span>Outs totali</span>
          </div>
          <strong>{totalOuts}</strong>
          <small>carte che migliorano la mano</small>
        </div>

        <div className="outs-probs">
          {turnPct !== null && (
            <ProbabilityMini label="Al Turn" value={turnPct} />
          )}
          <ProbabilityMini label={stage === 'flop' ? 'Turn + River' : 'Al River'} value={riverPct} />
        </div>
      </div>

      {outs.length > 0 && (
        <div>
          <p className="tiny-title">Carte che migliorano la mano</p>
      ...
      {totalOuts === 0 && (
        <div className="empty-state compact">
          <Info size={16} />
          Nessun out trovato oppure hai già un punto molto forte.
        </div>
      )}
    </div>
  );
