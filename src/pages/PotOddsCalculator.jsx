...
export default function PotOddsCalculator() {
  ...
  const data = useMemo(() => {
    ...
    const outsPct = street === 'flop'
      ? (1 - ((47 - outsNum) / 47) * ((46 - outsNum) / 46)) * 100
      : (outsNum / 46) * 100;
    return { finalPot, requiredEquity, profitable, ev, outsPct };
  ...
  return (
    <section className="page-card">
      <div className="section-header">
        <div>
          <span className="eyebrow"><Calculator size={14} /> {t('potEyebrow')}</span>
          <h2>{t('potTitle')}</h2>
          <p>{t('potText')}</p>
        </div>
      </div>
      ...
      <div className="tool-grid">
        <div className="panel form-panel">
          ...
          <label>{t('estimatedEquity')}<input type="number" min="0" max="100" value={equity} onChange={(e) => setEquity(e.target.value)} /></label>
          ...
          <label>
            ...
            <select value={street} onChange={(e) => setStreet(e.target.value)}>
              <option value="flop">{t('flopTwo')}</option>
              <option value="turn">{t('turnOne')}</option>
            </select>
          </label>
        </div>

        <div className="panel result-panel">
          <div className={data.profitable ? 'verdict good' : 'verdict bad'}>
            {data.profitable ? <CheckCircle2 size={26} /> : <XCircle size={26} />}
            <div>
              <span>{data.profitable ? t('mathOk') : t('notProfitable')}</span>
              <strong>{data.profitable ? t('callMakesSense') : t('callExpensive')}</strong>
            </div>
          </div>

          <div className="metric-list">
            <Metric label={t('requiredEquity')} value={`${data.requiredEquity.toFixed(1)}%`} />
            <Metric label={t('yourEquity')} value={`${Number(equity || 0).toFixed(1)}%`} />
            <Metric label={t('estimatedEv')} value={`${data.ev >= 0 ? '+' : ''}${data.ev.toFixed(2)}`} />
            <Metric label={t('outsProbability')} value={`${data.outsPct.toFixed(1)}%`} />
          </div>
