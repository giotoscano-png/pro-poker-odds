...
import { RotateCcw, Plus, Zap, Target, Info } from 'lucide-react';
    ...
    <section className="page-card">
      <div className="section-header">
        <div>
          ...
          <p>Inserisci carte personali, community cards e numero di giocatori.</p>
        </div>
      ...
      <motion.div className="panel" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
        ...
        <div className="card-row">
          ...
          {myHand.length < 2 && <span className="hint">Tocca per aggiungere</span>}
        </div>
      </motion.div>
      ...
      <motion.div className="panel" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
        <div className="tabs">
          <button className={activeTab === 'odds' ? 'tab active' : 'tab'} onClick={() => setActiveTab('odds')}>
            <Zap size={15} /> Probabilità {isCalculating ? '...' : ''}
          </button>
          <button className={activeTab === 'outs' ? 'tab active' : 'tab'} onClick={() => setActiveTab('outs')}>
            <Target size={15} /> Outs {outsAnalysis ? `(${outsAnalysis.totalOuts})` : ''}
          </button>
        </div>
        ...
        {activeTab === 'odds' ? (
          myHand.length < 2 ? (
            <div className="empty-state">
              <Info size={18} />
              Inserisci le tue 2 carte per calcolare le probabilità.
            </div>
          ) : (
      ...
      <p className="microcopy">
        Simulazione Monte Carlo • {odds?.iterations || 5000} iterazioni • giocatori totali incluso te.
      </p>
